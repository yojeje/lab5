...
// MercenariosClient is the client API for Mercenarios service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type MercenariosClient interface {
	PrepararMision(ctx context.Context, in *PreparacionRequest, opts ...grpc.CallOption) (*PreparacionResponse, error)
	TomarDecision(ctx context.Context, in *DecisionRequest, opts ...grpc.CallOption) (*DecisionResponse, error)
}

type mercenariosClient struct {
	cc grpc.ClientConnInterface
}

func NewMercenariosClient(cc grpc.ClientConnInterface) MercenariosClient {
	return &mercenariosClient{cc}
}

func (c *mercenariosClient) PrepararMision(ctx context.Context, in *PreparacionRequest, opts ...grpc.CallOption) (*PreparacionResponse, error) {
	out := new(PreparacionResponse)
	err := c.cc.Invoke(ctx, "/mission.Mercenarios/PrepararMision", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *mercenariosClient) TomarDecision(ctx context.Context, in *DecisionRequest, opts ...grpc.CallOption) (*DecisionResponse, error) {
	out := new(DecisionResponse)
	err := c.cc.Invoke(ctx, "/mission.Mercenarios/TomarDecision", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// MercenariosServer is the server API for Mercenarios service.
// All implementations must embed UnimplementedMercenariosServer
// for forward compatibility
type MercenariosServer interface {
	PrepararMision(context.Context, *PreparacionRequest) (*PreparacionResponse, error)
	TomarDecision(context.Context, *DecisionRequest) (*DecisionResponse, error)
	mustEmbedUnimplementedMercenariosServer()
}

// UnimplementedMercenariosServer must be embedded to have forward compatible implementations.
type UnimplementedMercenariosServer struct {
}

func (UnimplementedMercenariosServer) PrepararMision(context.Context, *PreparacionRequest) (*PreparacionResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method PrepararMision not implemented")
}
func (UnimplementedMercenariosServer) TomarDecision(context.Context, *DecisionRequest) (*DecisionResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method TomarDecision not implemented")
}
func (UnimplementedMercenariosServer) mustEmbedUnimplementedMercenariosServer() {}

// UnsafeMercenariosServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to MercenariosServer will
// result in compilation errors.
type UnsafeMercenariosServer interface {
	mustEmbedUnimplementedMercenariosServer()
}

func RegisterMercenariosServer(s grpc.ServiceRegistrar, srv MercenariosServer) {
	s.RegisterService(&Mercenarios_ServiceDesc, srv)
}

func _Mercenarios_PrepararMision_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(PreparacionRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MercenariosServer).PrepararMision(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/mission.Mercenarios/PrepararMision",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MercenariosServer).PrepararMision(ctx, req.(*PreparacionRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _Mercenarios_TomarDecision_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(DecisionRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MercenariosServer).TomarDecision(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/mission.Mercenarios/TomarDecision",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MercenariosServer).TomarDecision(ctx, req.(*DecisionRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// Mercenarios_ServiceDesc is the grpc.ServiceDesc for Mercenarios service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var Mercenarios_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "mission.Mercenarios",
	HandlerType: (*MercenariosServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "PrepararMision",
			Handler:    _Mercenarios_PrepararMision_Handler,
		},
		{
			MethodName: "TomarDecision",
			Handler:    _Mercenarios_TomarDecision_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "mision.proto",
